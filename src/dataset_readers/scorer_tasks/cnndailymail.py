...
class CNNDailyMailScorerTask:
    name = "cnndailymail"
    question_field = "article"
    prompt_field = "ctxs"

    def __init__(self, example_file, ds_size=None) -> None:
        if os.path.exists("/remote-home/klv/exps/rtv_icl/data"):
            dataset = load_from_disk("/remote-home/klv/exps/rtv_icl/data/cnndailymail")
        else:
    ...
    def get_fields(self, entry, index=-1):
        question_prefix = ""
        answer_prefix = "TL;DR: "
        test_question = question_prefix + entry['test_article']
        question = question_prefix + entry['article']
        decomposition = answer_prefix + entry['highlights']
        test_decomposition = entry['test_highlights']
