...
def get_question(entry):
    ...
    return WNLIBM25Task.norm(question)


@field_getter.add("qa")
def get_qa(entry):
    question = entry["sentence"]
    target = get_one_prompt('wnli', 0, entry['label'])
    return WNLIBM25Task.norm(question + " " + target)


@field_getter.add("a")
def get_decomp(entry):
    target = get_one_prompt('wnli', 0, entry['label'])
    return WNLIBM25Task.norm(target)
...
class WNLIBM25Task:
    ...
    def __init__(self, dataset_split, setup_type, ds_size=None):
        ...
        if os.path.exists("/remote-home/klv/exps/rtv_icl/data/wnli"):
            dataset = load_from_disk("/remote-home/klv/exps/rtv_icl/data/wnli")
        else:
