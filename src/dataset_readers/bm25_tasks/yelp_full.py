...
def get_question(entry):
    ...
    return YelpFullBM25Task.norm(question)


@field_getter.add("qa")
def get_qa(entry):
    question = entry["sentence"]
    target = get_one_prompt('yelp_full', 1, entry['label'])
    return YelpFullBM25Task.norm(question + " " + target)


@field_getter.add("a")
def get_decomp(entry):
    target = get_one_prompt('yelp_full', 1, entry['label'])
    return YelpFullBM25Task.norm(target)


class YelpFullBM25Task:
    name = 'yelp_full'
    ...
    def __init__(self, dataset_split, setup_type, ds_size=None):
        ...
        if os.path.exists("/remote-home/klv/exps/rtv_icl/data/yelp_full"):
            dataset = load_from_disk("/remote-home/klv/exps/rtv_icl/data/yelp_full")
        else:
