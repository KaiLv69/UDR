...
def get_question(entry):
    ...
    return WMTDeEnBM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return WMTDeEnBM25Task.norm(f"{entry['question']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return WMTDeEnBM25Task.norm(entry['target'])


class WMTDeEnBM25Task:
    name = 'wmt_de_en'
