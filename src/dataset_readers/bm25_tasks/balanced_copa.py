...
def get_question(entry):
    ...
    return BalancedCopaBM25Task.norm(question)
...
def get_qa(entry):
    question = entry["question"]
    target = entry['label']
    return BalancedCopaBM25Task.norm(question + " " + target)


@field_getter.add("a")
def get_decomp(entry):
    target = entry['label']
    return BalancedCopaBM25Task.norm(target)


class BalancedCopaBM25Task:
    name = 'balanced_copa'
    ...
    def __init__(self, dataset_split, setup_type, ds_size=None):
        ...
        if os.path.exists("/remote-home/klv/exps/rtv_icl/data/balanced_copa"):
            dataset = load_from_disk("/remote-home/klv/exps/rtv_icl/data/balanced_copa")
        else:
