...
def get_question(entry):
    ...
    return EmotionBM25Task.norm(question)


@field_getter.add("qa")
def get_qa(entry):
    question = entry["sentence"]
    target = get_one_prompt('emotion', 1, entry['label'])
    return EmotionBM25Task.norm(question + " " + target)


@field_getter.add("a")
def get_decomp(entry):
    target = get_one_prompt('emotion', 1, entry['label'])
    return EmotionBM25Task.norm(target)


class EmotionBM25Task:
    name = 'emotion'
