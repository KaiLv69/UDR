...
def get_question(entry):
    ...
    return WMTEnDeBM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return WMTEnDeBM25Task.norm(f"{entry['question']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return WMTEnDeBM25Task.norm(entry['target'])


class WMTEnDeBM25Task:
    name = 'wmt_en_de'
