...
def get_question(entry):
    ...
    return ArcEasyBM25Task.norm(question)
...
def get_qa(entry):
    question = entry["question"]
    target = entry['label']
    return ArcEasyBM25Task.norm(question + " " + target)


@field_getter.add("a")
def get_decomp(entry):
    target = entry['label']
    return ArcEasyBM25Task.norm(target)


class ArcEasyBM25Task:
    name = 'arc_easy'
    ...
    def __init__(self, dataset_split, setup_type, ds_size=None):
        ...
        if os.path.exists("/remote-home/klv/exps/rtv_icl/data/arc_easy"):
            dataset = load_from_disk("/remote-home/klv/exps/rtv_icl/data/arc_easy")
        else:
