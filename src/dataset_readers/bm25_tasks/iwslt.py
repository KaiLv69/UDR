...
def get_question(entry):
    ...
    return IWSLTBM25Task.norm(entry['translation.de'])


@field_getter.add("qa")
def get_qa(entry):
    return IWSLTBM25Task.norm(f"{entry['translation.de']} {entry['translation.en']}")


@field_getter.add("a")
def get_decomp(entry):
    return IWSLTBM25Task.norm(entry['translation.en'])
