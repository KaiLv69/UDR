...
def get_question(entry):
    ...
    return Bank77BM25Task.norm(question)


@field_getter.add("qa")
def get_qa(entry):
    question = entry["sentence"]
    target = get_one_prompt('bank77', 0, entry['label'])
    return Bank77BM25Task.norm(question + " " + target)


@field_getter.add("a")
def get_decomp(entry):
    target = get_one_prompt('bank77', 0, entry['label'])
    return Bank77BM25Task.norm(target)


class Bank77BM25Task:
    name = 'bank77'
    ...
    def __init__(self, dataset_split, setup_type, ds_size=None):
        ...
        if os.path.exists("/remote-home/klv/exps/rtv_icl/data/amazon"):
            dataset = load_from_disk("/remote-home/klv/exps/rtv_icl/data/bank77")
        else:
