...
def get_question(entry):
    ...
    return OpusparcusBM25Task.norm(entry['input'])


@field_getter.add("qa")
def get_qa(entry):
    return OpusparcusBM25Task.norm(f"{entry['input']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return OpusparcusBM25Task.norm(entry['target'])


class OpusparcusBM25Task:
    name = 'opusparcus'
