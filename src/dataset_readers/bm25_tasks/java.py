...
def get_question(entry):
    ...
    return JavaBM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return JavaBM25Task.norm(f"{entry['question']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return JavaBM25Task.norm(entry['target'])
...
class JavaBM25Task:
    ...
    def __init__(self, dataset_split, setup_type, ds_size=None):
        ...
        if os.path.exists("/remote-home/klv/exps/rtv_icl/data/java"):
            dataset = load_from_disk("/remote-home/klv/exps/rtv_icl/data/java")
        else:
