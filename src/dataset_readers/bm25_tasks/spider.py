...
def get_question(entry):
    ...
    return SpiderBM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return SpiderBM25Task.norm(f"{entry['question']} {entry['query']}")


@field_getter.add("a")
def get_decomp(entry):
    return SpiderBM25Task.norm(entry['query'])


class SpiderBM25Task:
    name = 'spider'
