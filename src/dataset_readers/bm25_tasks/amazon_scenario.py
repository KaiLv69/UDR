...
def get_question(entry):
    ...
    return AmazonScenarioBM25Task.norm(question)


@field_getter.add("qa")
def get_qa(entry):
    question = entry["sentence"]
    target = get_one_prompt('amazon_scenario', 0, entry['label'])
    return AmazonScenarioBM25Task.norm(question + " " + target)


@field_getter.add("a")
def get_decomp(entry):
    target = get_one_prompt('amazon_scenario', 0, entry['label'])
    return AmazonScenarioBM25Task.norm(target)


class AmazonScenarioBM25Task:
    name = 'amazon_scenario'
    ...
    def __init__(self, dataset_split, setup_type, ds_size=None):
        ...
        if os.path.exists("/remote-home/klv/exps/rtv_icl/data/amazon_scenario"):
            dataset = load_from_disk("/remote-home/klv/exps/rtv_icl/data/amazon_scenario")
        else:
