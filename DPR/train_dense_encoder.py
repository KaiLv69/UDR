...
class BiEncoderTrainer(object):
    ...
    def run_train(self):
        # ds_cfg = self.ds_cfg.train_datasets[0]
        # logger.info('ds_cfg:{}'.format(ds_cfg))
        # logger.info('encoder_type:{}'.format(ds_cfg.encoder_type))
        # exit()
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(
            ...
            if cfg.fp16:
                ...
                loss.backward()
                # logger.info('alert for debug, cancel the gradient norm clip')
                if cfg.train.max_grad_norm > 0:
                    torch.nn.utils.clip_grad_norm_(
            ...
            if (i + 1) % cfg.train.gradient_accumulation_steps == 0:
                ...
                #     elif self.loss_type == 'list_ranking':
                #         logger.info(
                #             "Epoch: %d: Step: %d/%d, loss=%f, rank_loss=%f, ib_loss=%f, lr=%f",
                #             epoch,
                #             updated_step,
                #             int(epoch_batches/cfg.train.gradient_accumulation_steps),
                #             step_loss,
...
def _do_biencoder_fwd_pass_list_ranking(
    ...
    # logger.info('local_q_vector:{}'.format(local_q_vector.size()))
    # logger.info('local_ctx_vectors:{}'.format(local_ctx_vectors.size()))
    # exit()
