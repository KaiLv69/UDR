...
for ds in "${gen_valid_datasets[*]}" "${gen_test_datasets[*]}" "${cls_valid_datasets[*]}" "${cls_test_datasets[*]}" "mnli"; do
...
for dataset in "${datasets[@]}"; do
  for split in "${splits[@]}"; do
    ctxs_fp="$PWD/exps/$exp_name/ctx_data/random_${dataset}_${split}.json"

    if [ ! -f "$ctxs_fp" ]; then
      echo "generating contexts for $dataset $split"
      python find_random.py \
      output_path="$ctxs_fp" \
      dataset_split="$split" \
      task_name="$dataset"
    fi
    ...
    --exp_name ${exp_name} --iter_num -1 --epoch_num -1 --method "random" \
