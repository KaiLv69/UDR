...
cuda_device="cuda:0"
num_gpus=1
exp_name="instructor_embed_0102"
...
cls_test_datasets=("yelp_full")
cls_valid_datasets=("cosmos_qa")
gen_test_datasets=()
gen_valid_datasets=()
main_process_port=23115
num_prompts=-1

mkdir -p "$PWD/exps/$exp_name/dr_data"
mkdir -p "$PWD/exps/$exp_name/inf_data"

generate_embedding_batch_size=256
gen=True
split="test"
for ds in "${gen_valid_datasets[*]}" "${cls_test_datasets[*]}" "${cls_valid_datasets[*]}" "${gen_test_datasets[*]}"; do
...
for dataset in "${datasets[@]}"; do
  ...
  python create_index.py \
    ...
    model._target_="src.models.instructor_embedder.IndexEmbedder" \
    task_name=${dataset}
  fi
  ...
  for split in "${splits[@]}"; do
    ...
    --exp_name ${exp_name} --iter_num -1 --epoch_num -1 --method "instructor_embed" \
