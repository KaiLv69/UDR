...
cls_test_datasets=("agnews" "amazon" "cola" "copa" "cr" "cs_explan" "cs_valid" "dbpedia" "mr" "snli" "sst2" \
"sst5" "subj" "trec" "yahoo" "yelp_full")
cls_valid_datasets=("qnli" "rte" "cosmos_qa")
...
mkdir -p "$PWD/exps/$exp_name/dr_data_iter0_all"
mkdir -p "$PWD/exps/$exp_name/inf_data_iter0_all"
...
for dataset in "${datasets[@]}"; do
  ...
  generate_dense_embeddings_py_output_path="$PWD/exps/$exp_name/dr_data_iter0_all/dpr_index_${dataset}_${epoch}_${step}"
...
#  fi
  ...
  for split in "${splits[@]}"; do
    ...
    dr_out="$PWD/exps/$exp_name/dr_data_iter0_all/dr_${dataset}_${split}_${epoch}_${step}"
#    if [ ! -f "$dr_out" ]; then
...
#    fi
    ...
    inference_out="$PWD/exps/$exp_name/inf_data_iter0_all/inference_${dataset}_${split}_${num_prompts}prompts_${epoch}_${step}.json"
...
#    fi
    ...
    --exp_name ${exp_name} --iter_num ${iter_num} --epoch_num ${epoch} --method ${step} \
