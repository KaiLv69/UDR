...
cvd=0,1,2,3,4,5,6,7
num_gpus=8
datasets=("cs_explan" "cs_valid" "copa" "cosmos_qa")
main_process_port=23901
...
mkdir -p "${sc_path}/${dataset}"
mkdir -p "${sc_path}/${dataset}/logs"
...
python DPR/generate_dense_embeddings.py \
     ...
     hydra.run.dir="${sc_path}/${dataset}/logs"
fi
...
in_scored_file="/nvme/xnli/lk_code/exps/rtv_icl/scored_data_iter0/${dataset}/${dataset}_${split}_scoredqa_merged.json"
