...
datasets=("break" "common_gen" "dart" "e2e" "mtop" "pubmed" "reddit" \
"roc_ending_generation" "roc_story_generation" "javascript")
datasets=('reddit' 'javascript')
split="train"
main_process_port=23990

#datasets=("smcalflow" "cnndailymail" "go" "java" "php" "python" "wikiauto")
#split="debug"
epoch=0
step=77041
...
out_file="${sc_path}/dr_converted_${dataset}_${split}_all"
out_dep_file="${sc_path}/dr_dep_${dataset}_${split}_all"
...
accelerate launch --num_processes $num_gpus --main_process_port ${main_process_port} --multi_gpu \
     ...
     hydra.run.dir="${sc_path}/logs"
fi

echo -e "\n\n-------merge-------\n\n"
merge_out_path="${sc_path}/${dataset}_${split}_scoredqa_merged_all.json"
